...
def run_picked_group_fdr_all(
    evidence_files, pout_files, fasta_file, output_dir, digest_params, input_type
):
    try:
        if len(output_dir) == 0:
...
def run_andromeda_to_pin(evidence_files, fasta_file, output_dir, digest_params):
    andromeda2pin.main(
        evidence_files
        + ["--outputTab", f"{output_dir}/pin.tab", "--databases", fasta_file]
        + digest_params
    )
...
def run_update_evidence(evidence_files, output_dir):
    update_evidence.main(
        ...
        + [
            "--perc_results",
            f"{output_dir}/mokapot.psms.txt",
            f"{output_dir}/mokapot.decoys.psms.txt",
            "--mq_evidence_out",
            f"{output_dir}/evidence_percolator.txt",
        ]
    )
...
def run_merge_pout(pout_files, fasta_file, output_dir, digest_params):
    merge_pout.main(
        ["--perc_results"]
        + pout_files
        + ["--perc_merged", f"{output_dir}/pout_merged.txt", "--fasta", fasta_file]
        + digest_params
    )
