...
class TestNoGrouping:
  def test_group_proteins(self, peptideInfoList):
    ...
    assert no_grouping.group_proteins(peptideInfoList, "") == ProteinGroups([["proteinA"], ["proteinB"], ["proteinC"]])


class TestSubsetGrouping:
  def test_group_proteins(self, peptideInfoList):
    subset_grouping = grouping.SubsetGrouping()
    assert subset_grouping.group_proteins(peptideInfoList, "") == ProteinGroups([["proteinA", "proteinB"], ["proteinC"]])

  def test_group_proteins_each_unique(self, peptideInfoListRescue):
    subset_grouping = grouping.SubsetGrouping()
    assert subset_grouping.group_proteins(peptideInfoListRescue, "") == ProteinGroups([["proteinA"], ["proteinB"], ["proteinC"]])
...
class TestRescuedSubsetGrouping:
  ...
  def test_scoring_threshold(self, proteinFdrResults):
    rescued_subset_grouping = grouping.RescuedSubsetGrouping()
    rescued_subset_grouping._calculate_rescue_score_cutoff(proteinFdrResults)
...
def peptideInfoList():
  ...
  """
  return { "PEPTIDEA" : (0.12, ["proteinA", "proteinB"]),
...
def peptideInfoListRescue():
  ...
  return { "PEPTIDEA" : (0.12, ["proteinA", "proteinB"]),
           ...
           "PEPTIDED" : (0.35, ["proteinB"]) } # the last two peptides should be removed by the rescuing procedure
