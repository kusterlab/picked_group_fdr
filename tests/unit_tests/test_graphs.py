...
class TestConnectedProteinGraphs:
  ...
  def test_decouple_connected_proteins_oneCutPossible(self):
    ...
    assert connectedProteinGraphs.decouple_connected_proteins(proteinGroups).protein_groups == [['proteinA', 'proteinE', 'proteinB'], ['proteinC', 'proteinD', 'proteinF']]
...
class TestPeptideProteinGraph:
  def test_getConnectedComponents(self):
    ...
    assert set([";".join(sorted(connectedProteinGraphs._get_protein_nodes(x))) for x in peptideProteinGraph.get_connected_components()]) == {'proteinA;proteinB', 'proteinC;proteinD'}
