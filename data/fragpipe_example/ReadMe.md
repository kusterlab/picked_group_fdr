...
4. In the `Validation` tab, in the `FDR Filter and Report` section, add the following in the `Filter` field.
   ```
   --prot 1.0 --ion 1.0 --pep 1.0 --psm 1.0
   ```
...
5. In the `Validation` tab, in the `FDR Filter and Report` section, tick both the `Do not use ProteinProphet file` and `Print decoys` checkbox.
